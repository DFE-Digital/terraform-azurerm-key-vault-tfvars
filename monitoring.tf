resource "azurerm_monitor_action_group" "main" {
  ...
  dynamic "logic_app_receiver" {
    for_each = local.monitor_logic_app_workflow.name != "" ? [0] : []

    content {
      name                    = local.monitor_logic_app_workflow_name
      resource_id             = local.monitor_logic_app_workflow_id
      callback_url            = local.monitor_logic_app_workflow_callback_url
      use_common_alert_schema = true
    }
  }
}
